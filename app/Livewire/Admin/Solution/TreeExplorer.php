...
class TreeExplorer extends Component
{
    ...
    public function loadTree()
    {
        ...
        if ($this->selectedTreeNodeId && isset($nodes[(int) $this->selectedTreeNodeId])) {
            return;
        }

        $this->selectedTreeNodeId = $roots[0] ?? null;
    }

    public function selectTreeNode($nodeId)
    {
        if (!$this->selectedProblem || !$nodeId) {
            return;
        }
        ...
        if ($exists) {
            $this->selectedTreeNodeId = (int) $nodeId;
        }
    }
    ...
    public function cloneFlow(QuestionFlowCloner $cloner)
    {
        ...
        if ($this->attachMode !== 'root' && !$this->targetAttachQuestionId && $this->selectedTreeNodeId) {
            $this->targetAttachQuestionId = (int) $this->selectedTreeNodeId;
        }
