                ...
                <div>
                    ...
                    <h3 class="text-3xl font-bold text-gray-800">
                        @php
                            ...
                            if ($total >= 10000000) {
                                echo '₹' . round($total / 10000000, 1) . 'Cr';
                            } elseif ($total >= 100000) {
                                echo '₹' . round($total / 100000, 1) . 'Lakh';
                            } elseif ($total >= 1000) {
                                echo '₹' . round($total / 1000, 1) . 'k';
                            } else {
        ...
        <div class="lg:col-span-2 bg-white rounded-xl shadow-md p-5 hover:shadow-lg transition duration-300">
            <div class="flex justify-between items-center mb-4">
                ...
                <a href="{{ route('frontdesk.servicerequest.manage') }}"
                    class="text-sm font-medium text-blue-600 hover:underline">View All</a>
            </div>
                        ...
                        @foreach ($recentServices as $service)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-500">
                                    {{ $service->owner_name }}</td>
                                <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-500">
                                    {{ $service->product_name }}</td>
                                <td class="px-4 py-3 whitespace-nowrap">
                                    @if ($service->status == 0)
                                        <span
                                            class="px-2 py-1 bg-blue-100 text-blue-800 rounded-full text-xs">Diagnosis</span>
                                    @elseif($service->status < 0.5)
                                        <span
                                            class="px-2 py-1 bg-yellow-100 text-yellow-800 rounded-full text-xs">Repair</span>
                                    @elseif($service->status < 1)
                                        <span
                                            class="px-2 py-1 bg-purple-100 text-purple-800 rounded-full text-xs">Quality
                                            Check</span>
                                    @else
                                        <span class="px-2 py-1 bg-green-100 text-green-800 rounded-full text-xs">Ready
                                            for Pickup</span>
                                    @endif
                                </td>
                                <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-500">
                                    {{ $service->estimate_delivery }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
                ...
                @foreach ($statusBreakdown as $status => $count)
                    <div>
                        ...
                        <div class="h-2 rounded-full bg-gray-200">
                            ...
                            @endphp
                            <div class="h-2 rounded-full {{ $color }}" style="width: {{ $percentage }}%">
                            </div>
                        </div>
                    </div>
                @endforeach
                ...
                @foreach ($statusBreakdown as $status => $count)
                    <div class="flex items-center gap-2">
                        ...
                        <span class="text-sm text-gray-600">{{ $status }}
                            ({{ round(($count / max(1, array_sum($statusBreakdown))) * 100) }}%)</span>
                    </div>
                @endforeach
            </div>
        </div>
            ...
            <div class="space-y-4">
                @foreach ($recentPayments as $payment)
                    <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                        <div>
                            <p class="text-sm font-medium text-gray-900">#{{ $payment->serviceRequest->service_code }}
                            </p>
                    ...
                    <div class="flex items-center space-x-3 p-3 hover:bg-gray-50 rounded-lg transition">
                        <div class="flex-shrink-0">
                            @if ($tech->image)
                                <img class="h-10 w-10 rounded-full"
                                    src="https://cdn-icons-png.flaticon.com/512/6342/6342703.png" alt="">
                            @else
                                <div
                        ...
                        <div class="text-right">
                            ...
                            <p class="text-xs text-gray-500">
                                {{ round(($tech->completed_services / max(1, $completedCount)) * 100) }}% share</p>
                        </div>
                    </div>
                @endforeach
            </div>
                ...
                <div class="flex flex-col justify-center space-y-3">
                    ...
                    @foreach ($deviceBreakdown as $device => $count)
                        <div>
                            ...
                            <div class="h-2 rounded-full bg-gray-200">
                                <div class="h-2 rounded-full {{ $colors[$loop->index] }}"
                                    style="width: {{ ($count / max(1, $totalDevices)) * 100 }}%"></div>
                            </div>
                        </div>
                    @endforeach
                </div>
