<div class="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-4 sm:py-6">
      ...
      <div class="bg-white rounded-xl shadow-md overflow-hidden">
        <div class="px-4 sm:px-6 py-3 sm:py-4 bg-gradient-to-r from-blue-50 to-indigo-50">
          <h2 class="text-lg sm:text-xl font-semibold text-gray-900 flex items-center">
            <i class="fas fa-user-tag text-indigo-600 mr-2 sm:mr-3 text-base sm:text-lg"></i> Customer &
            Product Information
          </h2>
        </div>
            ...
            <div class="space-y-3 sm:space-y-4">
              ...
              <div class="space-y-3">
                ...
                <div>
                  ...
                  <p class="mt-1 text-sm sm:text-base text-gray-900 font-medium">{{ $task->contact }}
                  </p>
                </div>
                <div>
            ...
            <div class="space-y-3 sm:space-y-4">
              ...
              <div class="space-y-3">
                ...
                <div>
                  ...
                  <p class="mt-1 text-sm sm:text-base text-gray-900 font-medium">{{ $task->brand }}
                  </p>
                </div>
                <div>
                  ...
                  <p class="mt-1 text-sm sm:text-base text-gray-900 font-medium">
                    {{ $task->serial_no ?? 'Not available' }}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="p-4 sm:p-6 space-y-4 sm:space-y-6">
          ...
          @if ($task->technician_notes)
            <div class="pt-4 border-t border-gray-200">
              <h3 class="text-xs sm:text-sm font-semibold text-gray-700 uppercase mb-2">Technician Notes
              </h3>
              <div class="bg-blue-50 p-3 sm:p-4 rounded-lg border border-blue-200">
                <p class="text-sm sm:text-base text-gray-800 leading-relaxed">
                  {{ $task->technician_notes }}</p>
              </div>
            </div>
          @endif
          @if ($task->remark)
            <div class="pt-4 border-t border-gray-200">
              <h3 class="text-xs sm:text-sm font-semibold text-gray-700 uppercase mb-2">Rejection Reason
              </h3>
    ...
    <div class="space-y-4 sm:space-y-6">
      ...
      <div class="bg-white rounded-xl shadow-md overflow-hidden">
        <div class="px-4 sm:px-6 py-3 sm:py-4 bg-gradient-to-r from-gray-50 to-gray-100">
          <h2 class="text-lg sm:text-xl font-semibold text-gray-900 flex items-center">
            <i class="fas fa-info-circle text-gray-600 mr-2 sm:mr-3 text-base sm:text-lg"></i> Service
            Information
          </h2>
        </div>
        <div class="p-4 sm:p-6 space-y-3 sm:space-y-4">
          <div class="flex items-start space-x-2 sm:space-x-3">
            ...
            <div>
              ...
              <p class="mt-1 text-sm sm:text-base text-gray-900 font-medium">
                {{ $task->serviceCategory->name ?? 'Not specified' }}</p>
            </div>
          </div>
          <div class="flex items-start space-x-2 sm:space-x-3">
          ...
          @if (!$paymentCompleted && !$taskRejected)
            ...
            <div class="mb-4 sm:mb-6">
              ...
              <div class="w-full bg-gray-200 rounded-full h-2.5 sm:h-3">
                <div class="bg-indigo-600 h-2.5 sm:h-3 rounded-full transition-colors duration-500 ease-in-out"
                  style="width: {{ $task->status }}%"></div>
              </div>
            </div>
            <!-- Reject Button -->
            <button wire:click="rejectTask"
              class="w-full flex items-center justify-center py-2.5 px-4 rounded-lg bg-red-600 text-white hover:bg-red-700 transition-colors duration-200 ">
              <i class="fas fa-times-circle mr-2"></i> Reject Task
              <svg wire:loading wire:target="rejectTask"
                class="animate-spin h-4 sm:h-5 w-4 sm:w-5 ml-2 text-white"
            ...
            <div class="mt-4 sm:mt-6">
              <label for="status" class="block text-xs font-medium text-gray-700 uppercase">Update
                Status</label>
              <div class="relative mt-1">
                <select id="status" wire:model="selectedStatus" wire:change="updateStatus"
                  class="block w-full py-2.5 px-3 text-sm border border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 transition-colors duration-200">
                ...
                <i
                  class="fas fa-chevron-down absolute right-3 top-1/2 -translate-y-1/2 text-gray-600 text-sm"></i>
              </div>
            </div>
          @elseif($taskRejected)
            ...
            @if ($task->delivery_status == '1')
              <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 w-full">
                ...
                <a wire:navigate href="{{ route('frontdesk.payment-details', $task->service_code) }}"
                  ...
                  class="inline-flex items-center justify-center px-4 py-2 text-sm sm:text-base bg-green-600 hover:bg-green-700 text-white font-medium rounded-md transition- w-full sm:w-auto text-center">
                ...
                @if ($task->status == '3' || ($task->payments->where('status', 'completed')->count() > 0 && $task->status == '2'))
                  <div class="flex flex-wrap gap-3">
                    <button wire:click="initiateDelivery"
                      class="flex items-center justify-center px-4 py-2 bg-green-600 text-white text-sm sm:text-base rounded-md hover:bg-green-700 transition-">
      ...
      @if ($showOtpModal)
        <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 px-3 sm:px-0">
          <div
            class="bg-white rounded-lg p-4 sm:p-6 w-full max-w-sm sm:max-w-md transition-colors duration-300">
            ...
            <div class="mb-3 sm:mb-4">
              <label class="block text-xs sm:text-sm text-gray-700 mb-2">Enter OTP sent to
                {{ $customerMobile }}</label>
              <input type="text" wire:model="otp"
                class="w-full px-3 py-2 text-sm border rounded-md focus:ring-indigo-500 focus:border-indigo-500 transition-colors duration-200"
                maxlength="6" @if ($otpTimeout) disabled @endif
                placeholder="6-digit OTP">
              <div class="mt-2 text-right">
                <span
                  class="text-xs sm:text-sm font-medium {{ $otpExpired ? 'text-red-600' : 'text-gray-600' }}"
                  id="otpTimer">
                  @if ($otpExpired)
                    OTP expired
                  @else
            ...
            <div class="flex flex-col sm:flex-row justify-between items-center gap-2 sm:gap-3">
              <button wire:click="verifyOtpAndMarkDelivered"
                class="w-full sm:w-auto px-3 sm:px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors duration-200 "
                @if ($otpTimeout) disabled @endif>
                Verify OTP
              </button>
              @if ($otpTimeout)
                <button wire:click="initiateDelivery"
                  class="w-full sm:w-auto px-3 sm:px-4 py-2 bg-green-600 text-white rounded-md hover:bg-green-700 transition-colors duration-200 ">
                  Resend OTP
                </button>
              @endif
              <button wire:click="$set('showOtpModal', false)"
                class="w-full sm:w-auto px-3 sm:px-4 py-2 bg-gray-600 text-white rounded-md hover:bg-gray-700 transition-colors duration-200 ">
                Cancel
              </button>
            </div>
          </div>
  ...
  @if ($showRejectionModal)
    <div class="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50 px-3 sm:px-0">
      <div
        class="bg-white rounded-xl shadow-xl max-w-sm sm:max-w-lg w-full p-4 sm:p-6 transition-colors duration-300">
        <div class="flex items-start">
          ...
          <div class="ml-3 sm:ml-4 flex-1">
            ...
            <p class="mt-2 text-xs sm:text-sm text-gray-600">Please provide a reason for rejecting this
              service request.</p>
            <textarea wire:model="rejectionReason" rows="4"
              class="mt-3 sm:mt-4 w-full p-3 text-sm border border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 transition-colors duration-200"
        ...
        <div class="mt-4 sm:mt-6 flex flex-col sm:flex-row justify-end gap-2 sm:gap-3">
          <button wire:click="confirmRejection"
            class="w-full sm:w-auto px-3 sm:px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors duration-200 ">Confirm
            Rejection</button>
          <button wire:click="$set('showRejectionModal', false)"
            class="w-full sm:w-auto px-3 sm:px-4 py-2 bg-white border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors duration-200 ">Cancel</button>
        </div>
      </div>
    </div>
  @endif
  ...
  @if (session()->has('delivery-success'))
    <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 3000)"
      class="fixed bottom-4 left-4 right-4 sm:left-auto sm:right-4 bg-green-600 text-white px-4 py-3 rounded-lg shadow-lg flex items-center text-sm sm:text-base transition-colors duration-300"
      x-transition:enter="transform ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4"
      x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transform ease-in duration-200"
