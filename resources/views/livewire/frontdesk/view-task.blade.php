            ...
            <div class="px-6 py-5 flex flex-col sm:flex-row sm:items-center sm:justify-between border-b border-gray-200">
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        ...
                        <div class="flex items-center mt-2 space-x-3">
                            <span
                                class="text-sm font-medium text-gray-600 bg-gray-200 px-3 py-1 rounded-full">SR-{{ $task->service_code }}</span>
                            @if ($paymentCompleted)
                                <span
                ...
                <div class="mt-4 sm:mt-0">
                    <span class="text-sm text-gray-600 bg-gray-100 px-4 py-2 rounded-full">
                        <i class="far fa-clock mr-1.5"></i> Last Updated:
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="space-y-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="mt-1 text-gray-900 font-medium">{{ $task->email ?? 'Not provided' }}
                                        </p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="mt-8 pt-6 border-t border-gray-200">
                                ...
                                <div class="relative w-full h-64 rounded-xl overflow-hidden shadow-md">
                                    ...
                                    <a href="{{ asset('storage/' . $task->image) }}" target="_blank"
                                        class="absolute bottom-4 right-4 bg-white p-2 rounded-full shadow-md hover:bg-gray-100 transition-colors">
                    ...
                    <div class="p-6 space-y-4">
                        ...
                        <div class="flex items-start space-x-3">
                            ...
                            <div>
                                ...
                                <p class="mt-1 text-gray-900 font-medium">{{ $task->receptionist->name ?? 'System' }}
                                </p>
                            </div>
                        </div>
                        <div class="flex items-start space-x-3">
                    ...
                    <div class="p-6">
                        @if (!$paymentCompleted && !$taskRejected)
                            ...
                            <button wire:click="rejectTask"
                                class="w-full flex items-center justify-center py-2.5 px-4 rounded-lg bg-red-600 text-white hover:bg-red-700 transition-colors">
                                <i class="fas fa-times-circle mr-2"></i> Reject Task
                                <svg wire:loading wire:target="rejectTask"
                                    class="animate-spin h-5 w-5 ml-2 text-white" xmlns="http://www.w3.org/2000/svg"
                                    ...
                                    <div class="space-y-4">
                                        <div>
                                            <label for="paymentAmount"
                                                class="block text-xs font-medium text-gray-700 uppercase">Amount
                                                (₹)</label>
                                            <div class="mt-1 relative rounded-md">
                                                ...
                                                <input type="number" wire:model="paymentAmount" id="paymentAmount"
                                                    class="block w-full pl-8 pr-4 py-2 border border-gray-300 rounded-md focus:ring-indigo-500 focus:border-indigo-500"
                                                    placeholder="0.00">
                                            </div>
                                        </div>
                                        <div>
                                            <label class="block text-xs font-medium text-gray-700 uppercase">Payment
                                                Method</label>
                                        ...
                                        <div>
                                            ...
                                            <input type="text" wire:model="paymentReference" id="paymentReference"
                                                class="block w-full py-2 px-3 border border-gray-300 rounded-md focus:ring-indigo-500 focus:border-indigo-500"
                                                placeholder="Enter payment reference">
                                        </div>
                                        <div class="flex space-x-3">
                                            <button wire:click="completeWithPayment"
                                                class="flex-1 bg-indigo-600 text-white py-2 rounded-lg hover:bg-indigo-700 transition-colors">Pay
                                                Now</button>
                                            <button wire:click="cancelPayment"
                                                class="flex-1 bg-white border border-gray-300 text-gray-700 py-2 rounded-lg hover:bg-gray-50 transition-colors">Cancel</button>
                                        </div>
                                    </div>
                                </div>
                            @endif
                            ...
                            <div class="mt-6">
                                ...
                                <div class="relative mt-1">
                                    <select id="status" wire:model="selectedStatus" wire:change="updateStatus"
                                        class="block w-full py-2.5 px-3 border border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500">
                                    ...
                                    <i
                                        class="fas fa-chevron-down absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-600"></i>
                                </div>
                            </div>
                        @elseif($taskRejected)
                                    ...
                                    <div>
                                        ...
                                        <div class="mt-3 space-y-2 text-sm text-gray-600">
                                            <div class="flex items-center">
                                                ...
                                                <span>Completed on
                                                    {{ $task->updated_at->format('M d, Y h:i A') }}</span>
                                            </div>
                            ...
                            @foreach ($task->payments as $payment)
                                <div class="border-b border-gray-200 pb-4 last:border-0">
                                    <div class="flex justify-between items-start">
                                        <div>
                                            <span
                                                class="font-bold text-gray-900">₹{{ number_format($payment->total_amount, 2) }}</span>
                                        ...
                                        <div class="text-right">
                                            <span
                                                class="text-xs text-gray-500">{{ $payment->created_at->format('M d, Y') }}</span>
                                            <div class="mt-1">
                                                <span
                                                    class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium 
                ...
                @if ($showOtpModal)
                    <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                        <div class="bg-white rounded-lg p-6 w-full max-w-md">
                            ...
                            <div class="mb-4">
                                <label class="block text-gray-700 mb-2">Enter OTP sent to
                                    {{ $customerMobile }}</label>
                                <input type="text" wire:model="otp" class="w-full px-3 py-2 border rounded-md"
                                ...
                                <div class="mt-2 text-right">
                                    <span
                                        class="text-sm font-medium {{ $otpExpired ? 'text-red-600' : 'text-gray-600' }}">
                                        @if ($otpExpired)
                                            OTP expired
                                        @else
                                            Time remaining: <span id="otpCountdown">{{ $countdownSeconds }}</span>
                                            seconds
                                        @endif
                                    </span>
                                </div>
            ...
            <div class="bg-white rounded-2xl shadow-xl max-w-lg w-full p-6">
                <div class="flex items-start">
                    ...
                    <div class="ml-4 flex-1">
                        ...
                        <p class="mt-2 text-sm text-gray-600">Please provide a reason for rejecting this service
                            request.</p>
                        <textarea wire:model="rejectionReason" rows="4"
                            class="mt-4 w-full p-3 border border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500"
                ...
                <div class="mt-6 flex justify-end space-x-3">
                    <button wire:click="confirmRejection"
                        class="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors">Confirm
                        Rejection</button>
