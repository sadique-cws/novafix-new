    ...
    @if($selectedUser && count($userAnswers) > 0)
        <div class="mt-5">
            <div class="flex justify-between items-center">
                ...
                <button wire:click="back" class="bg-gray-600 px-3 py-1 rounded font-medium text-white">Back</button>
            </div>
            ...
            <div class="flex justify-between mt-6">
                <button wire:click="prevAnswer"
                    class="px-5 py-2 bg-gray-600 text-white font-medium rounded-lg shadow transition hover:bg-gray-700 disabled:opacity-50"
                    @if($currentAnswerIndex == 0) disabled @endif>
                    &larr; Previous
                </button>
                @if($currentAnswerIndex == count($userAnswers) - 1)
                    {{ $selectedUser->id }}
                @endif
                <button wire:click="nextAnswer"
                    class="px-5 py-2 bg-blue-600 text-white font-medium rounded-lg shadow transition hover:bg-blue-700 disabled:opacity-50"
            ...
            <table class="w-full border border-gray-200 shadow-sm rounded-lg">
                <thead>
                    <tr class="bg-gray-100 text-gray-700 uppercase text-sm">
                        <th class="py-3 px-4 border">Staff Name</th>
