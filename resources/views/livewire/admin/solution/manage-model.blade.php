                ...
                <div class="bg-white rounded-xl shadow-2xs border border-slate-200 overflow-hidden">
                    <div class="bg-slate-50 px-5 py-4 border-b border-slate-200 flex items-center justify-between">
                        <div class="flex items-center gap-2">
                            <span class="p-1.5 rounded-lg {{ $editingModelId ? 'bg-amber-50 text-amber-600' : 'bg-blue-50 text-blue-600' }}">
                        ...
                        @if($editingModelId)
                            <span class="text-[11px] font-semibold text-amber-700 bg-amber-50 border border-amber-200 px-2 py-0.5 rounded-full">
                                Editing #{{ $editingModelId }}
                            </span>
                        @endif
                    </div>

                    <form class="p-5 space-y-4" wire:submit.prevent="{{ $editingModelId ? 'updateModel' : 'addModel' }}">
                        <div class="space-y-1.5">
                            <label class="block text-xs font-semibold text-slate-700">Select Brand <span class="text-rose-500">*</span></label>
                            ...
                            @error('brand_id') <p class="text-xs text-rose-600 font-medium mt-1">{{ $message }}</p> @enderror
                        </div>

                        <div class="space-y-1.5">
                            <label class="block text-xs font-semibold text-slate-700">Model Name <span class="text-rose-500">*</span></label>
                            <input type="text" wire:model="name" placeholder="e.g. iPhone 15 Pro, Galaxy S24"
                                class="block w-full px-3 py-2 text-xs sm:text-sm border border-slate-300 rounded-lg shadow-2xs bg-white text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-600 transition">
            ...
            <div class="w-full lg:w-2/3">
                <div class="bg-white rounded-xl shadow-2xs border border-slate-200 overflow-hidden">
                    ...
                    <div class="bg-slate-50 px-5 py-3.5 border-b border-slate-200 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                        <div class="flex items-center gap-2">
                            <h2 class="text-sm font-bold text-slate-800">Manage Models</h2>
                                    ...
                                    <tr class="border-b border-slate-200 hover:bg-blue-50/30 transition-colors">
                                        ...
                                        <td class="px-5 py-3.5 whitespace-nowrap border-b border-slate-200">
                                            <div class="flex items-center gap-2">
                                                ...
                                                <span class="text-xs sm:text-sm font-semibold text-slate-900">{{ $model->name }}</span>
                                            </div>
                                        </td>
                                    ...
                                    <tr>
                                        <td colspan="4" class="px-6 py-12 text-center border-b border-slate-200">
                                            <div class="max-w-sm mx-auto space-y-2">
                                                ...
                                                <p class="text-sm font-semibold text-slate-700">No models found</p>
                                                <p class="text-xs text-slate-400">Try adjusting your search query or add a new model using the form.</p>
                                            </div>
                                        </td>
                                    </tr>
                                @endforelse
