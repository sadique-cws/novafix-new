            ...
            <div class="bg-white rounded-lg shadow-md overflow-hidden">
                ...
                <form class="p-6 space-y-4" wire:submit.prevent="addModel">
                    <div class="space-y-2">
                        ...
                        <select wire:model="brand_id" class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-teal-500 focus:border-teal-500">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse ($models as $model)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ $model->brand->name }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $model->name }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <button wire:click="editModel({{ $model->id }})" class="mr-2 text-yellow-600 hover:text-yellow-900">
                                            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                                                <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z"/>
                                            </svg>
                                        </button>
                                        <button wire:click="deleteModel({{ $model->id }})" class="text-red-600 hover:text-red-900">
                                            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="px-6 py-4 text-center text-sm text-gray-500">No models found</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
